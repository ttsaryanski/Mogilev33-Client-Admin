...
export default function EditInvite() {
    ...
    useEffect(() => {
        ...
        const fetchInvite = async () => {
            try {
                const result = await inviteServices.getInviteById(
                    inviteId,
                    signal
                );
                ...
                if (!signal.aborted) {
                    setError(`Error fetching invite: ${error.message}`);
                }
            }
        };
    ...
    const submitHandler = async (e) => {
        ...
        setError(null);
        try {
            await inviteServices.editInviteById(inviteId, formData);

            navigate("/documents");
            clearForm();
        } catch (error) {
            setError(`Failed edit invite: ${error.message}`);
        } finally {
            setPending(false);
        }
    };
